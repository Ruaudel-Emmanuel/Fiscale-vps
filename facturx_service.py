...
def build_cii_xml(data):
    ...
    guideline = SubElement(
        context, f"{{{NS_RAM}}}GuidelineSpecifiedDocumentContextParameter"
    )
    add_text(
        guideline,
        f"{{{NS_RAM}}}ID",
    ...
    for idx, line in enumerate(data["lines"], start=1):
        ...
        line_item = SubElement(
            sctt, f"{{{NS_RAM}}}IncludedSupplyChainTradeLineItem"
        )
        line_doc = SubElement(
            line_item, f"{{{NS_RAM}}}AssociatedDocumentLineDocument"
        )
        ...
        billed_qty = SubElement(
            delivery, f"{{{NS_RAM}}}BilledQuantity", {"unitCode": "H87"}
        )
        ...
        summation = SubElement(
            settlement, f"{{{NS_RAM}}}SpecifiedTradeSettlementLineMonetarySummation"
        )
    ...
    add_text(
        seller_postal,
        f"{{{NS_RAM}}}LineOne",
        data.get("sellerAddress", "Adresse vendeur"),
    )
    ...
    add_text(
        buyer_postal,
        f"{{{NS_RAM}}}LineOne",
        data.get("buyerAddress", "Adresse client"),
    )
    ...
    if data.get("serviceDate"):
        event = SubElement(
            header_delivery, f"{{{NS_RAM}}}ActualDeliverySupplyChainEvent"
        )
        occurrence = SubElement(event, f"{{{NS_RAM}}}OccurrenceDateTime")
        add_text(
            occurrence,
            f"{{{NS_UDT}}}DateTimeString",
            parse_date_yyyymmdd(data["serviceDate"]),
            {"format": "102"},
        )
    ...
    add_text(
        ...
        fmt_dec(data["lines"][0].get("vatRate", 0) if data["lines"] else 0),
    )
    if exemption_reason:
        add_text(tax, f"{{{NS_RAM}}}ExemptionReason", exemption_reason)

    payment_terms = SubElement(
        header_settlement, f"{{{NS_RAM}}}SpecifiedTradePaymentTerms"
    )
    add_text(
        payment_terms,
        f"{{{NS_RAM}}}Description",
        data.get("paymentTerms", "30 jours"),
    )

    monetary = SubElement(
        header_settlement, f"{{{NS_RAM}}}SpecifiedTradeSettlementHeaderMonetarySummation"
    )
...
def build_visual_pdf(data, pdf_path):
    ...
    buyer = [
        data.get("buyerName", ""),
        data.get("buyerAddress", ""),
        f"SIREN: {data.get('buyerSiren', '')}",
    ]
    ...
    c.drawString(
        40,
        y,
        f"Paiement : {data.get('paymentMethod', '')} | Conditions : {data.get('paymentTerms', '')}",
    )

    if data.get("notes"):
        y -= 20
        c.drawString(40, y, f"Notes : {data.get('notes', '')}"[:95])
